<script lang="ts">
    ...
    $effect(() => {
        const button = buttons[11];
        if (!button) return;

        if (Platform.isMobile) {
            button.removeClass("button-border");
            if (subGitCount > 0) {
                button.addClass("button-border");
            }
        } else {
            button.firstElementChild?.removeAttribute("color");
            if (subGitCount > 0) {
                button.firstElementChild?.setAttr(
                    "color",
                    "var(--text-accent)"
                );
            }
    ...
    async function scanSubGit() {
        try {
            const entries = await plugin.subGitBridge.scan();
            const live = entries.filter((entry) => entry.state === "git");
            ...
            const details = [
                ...
                .join(", ");
            new Notice(
                `Found ${entries.length} child Git ${
                    entries.length === 1 ? "repository" : "repositories"
                }: ${details}. Commit-and-sync will bridge them temporarily.`,
                8000
            );
            console.info("[Git Subbridge] nested Git scan", entries);
        } catch (error) {
            const message =
